...
for (const [x1, z1, x2, z2] of clearRegions) {
  await sendCommand(rcon,
    `fill ${x1} ${cy} ${z1} ${x2} ${cy + clearHeight} ${z2} air`
  )
  await delay(150)
}    await delay(500)
    ...
    for (let i = -half; i <= half; i += 10) {
      await sendCommand(rcon, `setblock ${cx+i} ${torchY} ${cz-torchOffset} glowstone`)
      await sendCommand(rcon, `setblock ${cx+i} ${torchY} ${cz+torchOffset} glowstone`)
      await sendCommand(rcon, `setblock ${cx-torchOffset} ${torchY} ${cz+i} glowstone`)
      await sendCommand(rcon, `setblock ${cx+torchOffset} ${torchY} ${cz+i} glowstone`)
    }
    await delay(300)
